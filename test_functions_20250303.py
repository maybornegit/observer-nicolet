...
from models_test import EffNetV2640_C, EffNetV2640D, EffNetV2640RGB
...
def labelled_image(filename, weight):
    image_path = filename.replace('npy', 'png')
    ...
    cv2.rectangle(full_image, rectangle_top_left, rectangle_bottom_right, (0, 0, 0), -1)  # -1 to fill the rectangle
    full_image = cv2.putText(full_image, text, org, font, fontScale, color, thickness, cv2.LINE_AA)
    return full_image
