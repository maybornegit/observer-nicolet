...
# run_model(f, h, x0, u, p, t_span, dt, 'Base Case', file_id='k_IC_test')
t_traj, x_traj, y_traj = solve_system(f, h, x0, u, p, t_span, dt) # Basic run for model, no graphs
y_traj_true = y_traj.copy() # Copy of True Output Traj. without Noise


####### Noise Simulation ##########
 
### Noise Initialization
noise_percent = 10  # Noise Hyper-Parameter (Mean Prediction Error of x%)
noise = y_traj*np.random.normal(0,1.5,y_traj.shape[1])*noise_percent/100 # Gaussian noise so that the Mean Prediction Error is noise_percent
y_traj = y_traj+noise  # Add noise to trajectory

### Optional FFT for Analysis
# y = np.array(y_traj[0,:])
# y_ = fft(y)
# N = y_traj.shape[1]
# T = dt
...
# plt.semilogy(xf, 2.0/N * np.abs(y_[0:N//2]))
# plt.show()

### Noise Smoothing
b, a = signal.butter(7,.02,'low') # 7th Order Low-Pass Butterwoth Filter
y1 = signal.filtfilt(b, a, y_traj[0,:]) # Pass 1st Output Through Low-Pass
y2 = signal.filtfilt(b, a, y_traj[1,:]) # Pass 2nd Output Through Low-Pass
y1 = gaussian_filter1d(y1, 100) # Pass 1st Output Through Gaussian Smoothing
y2 = gaussian_filter1d(y2, 100) # Pass 2nd Output Through Gaussian Smoothing
y1 -= y1[0]-x0[0] # Initialize Starting Output as True Initial Condition
y2 -= y2[0]-x0[1] # Initialize Starting Output as True Initial Condition

## Plot 1st Output with Noise, without Noise, and Smoothed
# plt.plot(t_traj/60/60/24, y_traj[0,:], label='Trajectory with Noise')
plt.plot(t_traj/60/60/24, y_traj_true[0,:], label='Original Trajectory')
y_traj[0,:] = y1
plt.plot(t_traj/60/60/24, y_traj[0,:],'--', label='Smoothed Trajectory')
plt.legend(loc='lower right')
plt.title('Adding Noise to NICOLET, Dry Biomass')
plt.xlabel('Time (days)')
plt.ylabel('Dry Biomass (kg/m2)')
plt.grid(True)
plt.show()

## Plot 2nd Output with Noise, without Noise, and Smoothed
# plt.plot(t_traj/60/60/24, y_traj[1,:], label='Trajectory with Noise')
plt.plot(t_traj/60/60/24, y_traj_true[1,:], label='Original Trajectory')
y_traj[1,:] = y2
plt.plot(t_traj/60/60/24, y_traj[1,:],'--', label='Smoothed Trajectory')
plt.legend(loc='lower right')
plt.xlabel('Time (days)')
plt.ylabel('Fresh Biomass (kg/m2)')
plt.title('Adding Noise to NICOLET, Fresh Biomass')
plt.grid(True)
plt.show()
...
step_diff = 0.9         # Difference between actual parameter and initial guess
scale = 5e-10             # Real, Negative Part of Poles for Observer Placement
scale_im = 1e-8j         # Imaginary Part of Poles for Observer Placement
...
for i in range(1,x_traj.shape[1]):
    ...
    if i % skip_num == 0:
        ...
        guess_term = np.hstack((f(x_init[0:2],u,p, 0), np.array([0,0]))) # Derivative at Prev. State Estimate
        x_guess_0 = runge_kutta_4(f, x_init[0:2], u, p, dt,0)  # Get Next State from RK-4 Estimation
...
print(100*np.abs(1-np.mean(guesses[2,:-20])/true_value))
